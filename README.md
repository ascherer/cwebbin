...
drop](https://github.com/ascherer/cweb/releases/download/cweb-4.12.2/cweb-4.12.2.tar.gz)
...
extract `ctwill.tar.gz` and add the contents of `cweb-4.12.2.tar.gz` (overwriting
outdated source files `Makefile`, `common.h`, `common.w`, and `prod.w`) and
`cwebbin-2025.1.tar.gz` for the full set of source files.
replace `@@VERSION@@` in line 129 of the `Makefile.unix` with something like
`Version 4.12.2 [CWEBbin 2025.1]`.
`touch *.cxx`.
...
```
git archive -o cweb-4.12.2.tar.gz cweb-4.12.2
git archive -o cwebbin-2025.1.tar.gz cwebbin-2025.1
```
...
pax -rzf /path/to/cweb-4.12.2.tar.gz
```
